...
class SearchQueryManager(models.Manager):
    def get_recent_queries(self, days: int) -> list[str]:
        """Extract, clean, and deduplicate recent search queries."""
        try:
            cutoff_date = timezone.now() - timedelta(days=days)
            user_queries = self.filter(created_at__gte=cutoff_date).values_list(
                "query", flat=True
            )
            ...
            logger.info(
                f"Extracted {len(unique_queries)} unique user search queries for analysis"
            )
